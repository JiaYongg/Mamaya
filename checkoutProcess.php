...
if($_POST) //Post Data received from Shopping cart page.
{
	...
	foreach ($_SESSION["Items"] as $key => $value)
	{
		...
		$stmt->bind_param("i", $value["productId"]);
		$stmt->execute();
		...
		{
			echo "Product $value[productId] : $value[name] is out of stock!<br />";
	   ...
	   "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
	{
		...
		if ($result->num_rows > 0) 
		{
			while ($row = $result->fetch_array())
			{
				...
				$stockLeft = $row2["Quantity"] - $row["Quantity"];
				$qry = "UPDATE product SET Quantity = ? WHERE ProductID = $row[ProductID]";
				$stmt = $conn->prepare($qry);
				$stmt->bind_param("i", $stockLeft);
				$stmt->execute();
				$stmt->close();
			}
		}
